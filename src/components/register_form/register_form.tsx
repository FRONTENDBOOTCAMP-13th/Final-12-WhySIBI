...
export default function RegisterForm() {
  ...
  return (
    <>
      <div className="text-gray-500 mt-6 md:mt-8 mx-4 md:mx-auto max-w-[43.25rem] flex flex-row-reverse">
        <button
          type="button"
          onClick={() => {
            setRegisterType(registerType === 'seller' ? 'user' : 'seller');
          }}
          className="nahonsan-btn-3d-white rounded-full border-2 border-gray-300 font-basic font-bold bg-white p-2.5 cursor-pointer"
        >
          {registerType === 'seller'
            ? '일반회원 회원가입'
            : '판매자용 회원가입'}
      ...
      <form
        action={formAction}
        className="px-6 py-12 md:px-12 lg:px-20 md:py-16 lg:py-24"
      >
        <input type="hidden" name="type" value={registerType} />
