...
import OrderModal from '../order_state_modal/order_state_modal';
...
function SellerOrderInfo({
  ...
  const [orderDeliveryState, setOrderDeliveryState] = useState(false);
  const handleOrderStateModal = () => {
    setOrderDeliveryState(true);
  };
  return (
    <>
      ...
      {orderDeliveryState === true && (
        <OrderModal
          ...
          userName={user.name}
          deliveryState={state}
          onClose={() => setOrderDeliveryState(false)}
        />
      )}
    </>
  );
